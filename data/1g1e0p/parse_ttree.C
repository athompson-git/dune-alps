void parse_ttree() {
    const char* filename = "analysis_1g1em_run_4_in_numubar_flux_numubar_1M_events.root";
    const char* tree_name = "gtree_flat";

    TFile *file = new TFile(filename, "READ");
    TTree *tree = (TTree*)file->Get(tree_name);
    
    TBranch *p1em = tree->GetBranch("pxe");
    TBranch *p2em = tree->GetBranch("pye");
    TBranch *p3em = tree->GetBranch("pze");
    TBranch *p0em = tree->GetBranch("ee");

    TBranch *p1ep = tree->GetBranch("pxgamma");
    TBranch *p2ep = tree->GetBranch("pygamma");
    TBranch *p3ep = tree->GetBranch("pzgamma");
    TBranch *p0ep = tree->GetBranch("egamma");
    ...
    std::ofstream flux;
    flux.open("emgamma_numubar_4vectors_DUNE_bkg.txt");
